...
Options:
    ...
    -parentdir	STRING	name of dir representing top of tree in which to
                        search for files; default '$parent_dir'
...
print STDERR qq{$command
    ...
    in directory tree '$parent_dir/' . Buffer of protein_id-keyed lines
...
process_gff_dirs($parent_dir, $file_ext, $output_dest,
...
###    print qq{$file_ext_re\n};
    ...
    (my $parent_dir = $orig_parent_dir) =~ s{ [/] \z }{}xms;
    ...
    my @gff_files = @{ $file_list->find($file_ext_end) };
...
sub write_buffer_to_files {
    ...
    my @unique_keys = uniq (map { substr($_,0,$key_length) } @buffer_lines);
