...
while (defined (my $line=<$fh>) ) {
    ...
    if ( ($line =~ m{ \A \s* [#] }xms) || ($line !~ m{ \S }xms) ) {
        push @lines, $line;
        next;
    }
...
#####        while ($attribute_list =~ m{ \G ( $fname_regexp ) [=] [^;]+ (?: [;] | \z ) }gxms) {
        ...
        my @closing = ( ')', ']', '}' );
        my ($failures, @missing_closing) = check_parentheses($field_value);

        for my $i (0 .. $#closing) {
            # for the moment, can't deal with *excess* (rather than lack of)
            # closing parentheses, etc
            croak qq{(file $gff_file):problem with value '$field_value' (field $field_name)\n}.
              ...
              qq{in line:\n(file $gff_file):$line} if $missing_closing[$i] < 0;

            next if !$missing_closing[$i];

            $field_value .= $closing[$i] x $missing_closing[$i];
        }

        $data{$field_name} = $field_value;

        $new_attribute_list .= q{;} if ($new_attribute_list);
        $new_attribute_list .= qq{${field_name}=$field_value};
        
    }

    if ($new_attribute_list ne $attribute_list) {
        print STDERR qq{WARNING: (file $gff_file): corrected attributed list string in line:\n},
          qq{(file $gff_file):$line\nwas:\n'$attribute_list'\nnow:\n'$new_attribute_list'\n};
        print STDERR qq{(file $gff_file):corresponding protein_id: $data{protein_id}\n}
            if (defined $data{protein_id});
        $line = join(qq{\t}, @fields, $new_attribute_list);
        $n_corrected_lines++;
    }
...
print STDERR qq{$n_corrected_lines lines corrected\n};

if ($n_corrected_lines) {
    print join(qq{\n},@lines), qq{\n};    
}
...
sub check_parentheses {
    ...
    return ($n_problems, $parenthesis_problems, $bracket_problems, $brace_problems);
}
